...
- (void)wxRefund:(NSString*)outTradeNo {
    ...
    [BCWXPay reqWXRefundV3:outTradeNo outRefundNo:outRefundNo refundReason:kRefundReason refundFee:@"1" payBlock:^(BOOL success, NSString *strMsg, NSError *error) {
        if (success) {
            // 表明预退款成功
        } else {
...
//    NSString *outRefundNo = [[BCUtil generateRandomUUID] stringByReplacingOccurrencesOfString:@"-" withString:@""];
//    [BCUPersonalPay reqUnionRefund:outTradeNo refundFee:@"1" outRefundNo:outRefundNo refundReason:kRefundReason refundBlock:^(BOOL success, NSString *strMsg, NSError *error) {
//        if (success) {
//            //
//        }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *statusStr = [[[self.payList objectAtIndex:indexPath.row] objectForKey:trade_status_key] isEqualToString:@"TRADE_SUCCESS"]?@"1":@"0";
    cell.textLabel.text = [NSString stringWithFormat:@"%@交易状态:%@", [[self.payList objectAtIndex:indexPath.row] objectForKey:@"trace_time"],statusStr];
