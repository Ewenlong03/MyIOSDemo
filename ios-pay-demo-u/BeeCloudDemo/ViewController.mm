...
- (void)unionRefund:(NSString*)outTradeNo {
    NSString *outRefundNo = [[BCUtil generateRandomUUID] stringByReplacingOccurrencesOfString:@"-" withString:@""];
    [BCUnionPay reqUnionRefund:outTradeNo refundFee:@"1" outRefundNo:outRefundNo refundReason:kRefundReason refundBlock:^(BOOL success, NSString *strMsg, NSError *error) {
        if (success) {
            //
        }else{
